...
def create_model(inputs, config):
    ...
    with arg_scope([ops.conv2d, ops.fc], stddev=0.01, weight_decay=0.0005, ):
        with arg_scope([ops.conv2d], stddev=0.1, activation=tf.nn.relu, batch_norm_params=batch_norm_params):
            ...
            if not config.get("training_mode", False):
                end_points['dropout4'] = ops.dropout(end_points['pool4'], dropout_rate, scope='dropout4')
                previous = end_points['dropout4']
            else:
                previous = end_points['pool4']

            end_points['conv5'] = ops.conv2d(previous, 512, [3, 3], scope='conv5')
